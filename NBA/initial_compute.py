...
class ComputeTeams:
    ...
    def write_games_with_df(df):
        for indx,row in df.iterrows():
            object = Game.objects.create(game_id = row[DfColumnNames.GAME_ID.value],team_id = row[DfColumnNames.TEAM_ID.value + "_x"],game_date = row[DfColumnNames.GAME_DATE.value],opponent_id = row[DfColumnNames.TEAM_ID.value + "_y"],wl = row[DfColumnNames.WL.value])
            object.save()
...
class ComputePlayers:
    ...
    def compute_player_elo_from_db(elo_diff):
        games = Game.objects.order_by(TableColumnNames.GAME_DATE.value)
        df = pd.DataFrame(games.values())
        ComputePlayers.write_player_elo_with_df(df,elo_diff)

    def write_player_elo_with_df(df,elo_diff):
